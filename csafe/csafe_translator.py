...
"""
EXTENDED_FRAME_START_FLAG = 0xF0    # 240 - You plan to control multiple PMs on a network OR racing.
STANDARD_FRAME_START_FLAG = 0xF1    # 241 - You're only communicating with one PM at a time.
...
class CSAFEMessageHandler(object):
    ...
    def _from_csafe(self, extended_frame, pm_response: list):
        
        # Trim the padding bytes
        # TODO: Need to trim the response padding to remove padding bytes 0x00
        
        # Check for valid frame start and end flags
        if pm_response[0] != STANDARD_FRAME_START_FLAG or pm_response[-1] != STOP_FRAME_FLAG:
            raise ValueError("Invalid CSAFE frame format")

        # Process for Standard Frame

        # Process for Extended Frame

        # Check sum for the message
    ...
    def pm_message_sender(self, device, command, extended_frame=False, pretty=False):
        """
        ...
        csafe = self._to_csafe(host_arguments=command, extended_frame=extended_frame)
        ...
        try:
            ...
            if not transmission or len(transmission) < 3:
                logging.error(f"No or incomplete transmission received. {transmission}")
                return None
